...
export const SearchBar = ({ setResults }) => {
    ...
    fetch(`https://api.waqi.info/search/?keyword=stockholm&token=a3bf1197881754e07fb1a334116289ffb6104296`)
      ...
      .then(data => {
        const results = data.data.filter(result => {
          ...
          // For example, filtering based on the name of the station
          return value && result.station.name && result.station.name.toLowerCase().includes(value)
        })
        setResults(results) // Log the filtered results
        // Do something with the filtered results, such as updating state
      })
      .catch(error => {
  ...
  return (
    <div className="input-wrapper">
      ...
      <input placeholder="Search Location..." value={input} onChange={e => handleChange(e.target.value)} />
    </div>
  )
}
